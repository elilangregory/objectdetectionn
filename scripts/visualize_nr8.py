"""
Visualize nr8 pipeline on test images.
Shows gate status: GREEN=centroid confident, YELLOW=gate-assisted, no box=dropped.

Usage:
    cd /Users/elilangregory/Documents/NM_AI/object_detection
    venv/bin/python scripts/visualize_nr8.py --n 25
"""
...
def detect_single(session, input_name, image, imgsz, det_conf):
    ...
    keep=scores>=det_conf;boxes,scores=boxes[keep],scores[keep]
...
def embed_all(model, images, device):
    ...
    for i in range(0,len(images),BATCH):
        batch_pil=images[i:i+BATCH];aug=[]
...
def weighted_box_fusion(boxes_list, scores_list, iou_threshold):
    ...
    order=np.argsort(np.asarray(scores_list))[::-1];clusters=[]
    for idx in order.tolist():
        box,score=boxes_list[idx],float(scores_list[idx])
        match,best_iou=None,0.0
        for c in clusters:
            overlap=float(iou_vector(box,c["box"][None,:])[0])
            if overlap>=iou_threshold and overlap>best_iou:best_iou=overlap;match=c
        if match is None:clusters.append({"box":box.copy(),"weight":score,"scores":[score]})
        else:
            w=match["weight"]+score
            match["box"]=(match["box"]*match["weight"]+box*score)/max(w,1e-8)
            match["weight"]=w;match["scores"].append(score)
    return[{"box":c["box"],"score":max(c["scores"])}for c in sorted(clusters,key=lambda c:max(c["scores"]),reverse=True)]
...
def main(args):
    ...
    det_sess = ort.InferenceSession("submission_nr8/third_medium_best.onnx", providers=providers)
    det_input = det_sess.get_inputs()[0].name

    print("Loading multiclass gate ...")
    cls_sess = ort.InferenceSession("submission_nr8/classifier.onnx", providers=providers)
    cls_input = cls_sess.get_inputs()[0].name

    ref_protos, ref_mask = None, None
    ref_path = Path("submission_nr8/reference_prototypes.json")
    if ref_path.exists():
        with ref_path.open() as f:
            rd = json.load(f)
        ref_protos = np.asarray(rd["prototypes"], dtype=np.float32)
        ref_mask = np.asarray(rd["mask"], dtype=bool)

    print("Loading DINOv2 ...")
    dinov2 = load_dinov2("dinov2_finetuned.pt", device)

    print("Loading centroids ...")
    with open("submission_nr8/centroids.json") as f:
    ...
    total_kept, total_dropped, total_gate_assisted = 0, 0, 0
    ...
    for img_path in selected:
        ...
        all_boxes, all_scores = detect_single(det_sess, det_input, img_pil, 960, args.det_conf)
        if not all_boxes:
        ...
        for det in fused:
            ...
            w,h = x2-x1, y2-y1
            px,py = w*0.12, h*0.12
            left=max(0,int(math.floor(x1-px)));top=max(0,int(math.floor(y1-py)))
            right=min(iw,int(math.ceil(x2+px)));bottom=min(ih,int(math.ceil(y2+py)))
            if right-left<5 or bottom-top<5:continue
            crops.append(img_pil.crop((left,top,right,bottom)))
            det_meta.append(det)

        if not crops:
            print("  No valid crops"); continue

        # Gate confidence
        gate_confs = []
        for start in range(0, len(crops), 64):
            batch = crops[start:start+64]
            resize_sz = int(288*1.14)
            batch_arr = []
            for img in batch:
                resized = img.resize((resize_sz,resize_sz),Image.BILINEAR)
                left=max((resize_sz-288)//2,0);top=max((resize_sz-288)//2,0)
                cropped = resized.crop((left,top,left+288,top+288))
                arr = np.asarray(cropped,dtype=np.float32)/255.0
                arr = (arr-MEAN)/STD
                batch_arr.append(np.transpose(arr,(2,0,1)))
            batch_np = np.stack(batch_arr).astype(np.float32)
            logits, embeddings = cls_sess.run(None, {cls_input: batch_np})
            probs = softmax(logits)
            if ref_protos is not None and ref_mask is not None:
                norms = np.linalg.norm(embeddings, axis=1, keepdims=True)
                embs_n = embeddings / np.clip(norms, 1e-8, None)
                sim = np.nan_to_num(embs_n @ ref_protos.T)
                sim = (sim + 1.0) / 2.0
                fused_p = 0.7 * probs + 0.3 * sim * ref_mask[None, :]
            else:
                fused_p = probs
            gate_confs.extend(fused_p.max(axis=1).tolist())
        ...
        kept, dropped, gate_assisted = 0, 0, 0
        for i, det in enumerate(det_meta):
            best_idx = int(np.argmax(sims[i]))
            best_sim = float(sims[i][best_idx])
            gate_conf = gate_confs[i]

            if best_sim >= args.cls_conf:
                color = (0, 200, 0)  # green — centroid confident
            elif gate_conf >= args.gate_thresh and best_sim >= 0.35:
                color = (0, 200, 255)  # yellow — gate assisted
                gate_assisted += 1
            else:
                dropped += 1
                continue
        ...
        total_kept += kept; total_dropped += dropped; total_gate_assisted += gate_assisted

        cv2.putText(bgr, f"{img_path.name}  kept={kept} gate={gate_assisted} dropped={dropped}",
                    (10,30), font, 0.7, (255,255,255), 2)
        cv2.putText(bgr, f"GREEN=centroid  YELLOW=gate-assisted  det={args.det_conf} cls={args.cls_conf} gate={args.gate_thresh}",
                    (10,55), font, 0.4, (200,200,200), 1)

        out_path = out_dir / img_path.name
        cv2.imwrite(str(out_path), bgr)
        print(f"  Kept: {kept}  Gate-assisted: {gate_assisted}  Dropped: {dropped}")

    print(f"\nDone. Kept: {total_kept}  Gate-assisted: {total_gate_assisted}  Dropped: {total_dropped}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="output_test_vis_nr8")
    parser.add_argument("--det-conf", dest="det_conf", type=float, default=0.10)
    parser.add_argument("--cls-conf", dest="cls_conf", type=float, default=0.50)
    parser.add_argument("--gate-thresh", dest="gate_thresh", type=float, default=0.15)
